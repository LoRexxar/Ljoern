...
class ModuleFunctionCpgTests extends PySrc2CpgFixture with Matchers {
  ...
  "test module method node properties" in {
    val methodNode = cpg.method.fullName("test.<module>").head
    methodNode.name shouldBe "<module>"
    methodNode.fullName shouldBe "test.<module>"
    methodNode.lineNumber shouldBe Some(1)
  }

  "test method body" in {
    val topLevelExprs = cpg.method.fullName("test.<module>").topLevelExpressions.l
