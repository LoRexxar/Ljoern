...
class BytecodeIndexedClassPathTests extends AnyFreeSpec with Matchers with BeforeAndAfterAll {
  ...
  "BytecodeIndexedClassPath" - {
    ...
    "should resolve classes in a standard JAR by their actual names" in {
      jarTestWithEntries(
        ...
        val classPath = new BytecodeIndexedClassPath(jarPath.toString)
    ...
    "should handle classes in the default package" in {
      jarTestWithEntries(ClassEntry("RootClass", "RootClass.class")) { jarPath =>
        val classPath = new BytecodeIndexedClassPath(jarPath.toString)
  ...
  private def jarTestWithEntries(classEntries: ClassEntry*)(testFn: Path => Unit): Unit = {
    FileUtil.usingTemporaryDirectory("jar-classpath-test") { tmpDir =>
      val jarPath = createJarWithClasses(tmpDir, "test.jar", classEntries)
  ...
  /** Creates a JAR file containing class files at the specified entry paths, where each class file's bytecode declares
    ...
    */
  private def createJarWithClasses(tempDir: Path, jarName: String, classEntries: Seq[ClassEntry]): Path = {
